...
Metadata is structured information about digital objects. Without metadata, a digital file is just an opaque sequence of bits: you cannot find it, you cannot understand what it represents, and you cannot prove it is what it claims to be. Metadata exists to enable the preservation of data; for an overview of how the DPS handles data, see [Data management](/docs/dps/data/).

For digital preservation, metadata serves several essential functions:

- It enables four fundamental user tasks: **find** content matching your criteria, **identify** the right object among similar results, **select** the appropriate representation, and **obtain** the actual content through dissemination
...
- It provides **context**: how the content relates to other objects in (or outside) the collection

Rich metadata is not created all at once. It accumulates over the lifetime of a digital object: some metadata are provided by a client at submission, others are produced automatically during the ingest by the DPS, and others build up over decades of preservation activities. 

Some metadata layers are designed for enrichment over time, while others are deliberate snapshots frozen at the point of submission. Each layer serves a different purpose and is created at a different stage. Together, these layers form a complete picture of every digital object in our care.
...
Many client partners already maintain rich cataloguing and metadata systems. A natural question is: if the content is already well described in their systems, why do they need to provide metadata again through the DPS?

The answer lies in how access to preserved content works. Access to the DPS is governed by [submission agreements and consumer roles](/docs/dps/access-control/). A client who submits content (producer) may not be the same client who retrieves it (consumer). For example:
...
The richer metadata you pack into SIPs is still preserved and available through dissemination, but a consumer must first be able to find the right package to request to access them. That is what the searchable layers provide.

If you already maintain such metadata in your own cataloguing systems, providing it through the Submission API is typically straightforward. The Dublin Core fields are well-defined, and in many cases mapping from your existing metadata formats is a one-time configuration effort.

Ultimately, the metadata you provide in the DPS enables a higher degree of self-service. Authorized consumers can find and retrieve the content they are after without having to go through intermediaries, whether that is the original submitter, a catalogue team, or a preservation custodian. The searchable and retrievable metadata layers together make the DPS a self-contained platform for discovery and access.

The DPS is first and foremost a system for safeguarding data (see [Data management](/docs/dps/data/)). Metadata is what makes that data findable, trustworthy, and usable over time.

## How metadata enters the DPS

Each type of metadata serves a specific purpose in the preservation lifecycle, from making content findable, to building an audit trail, to preserving the context of a submission. The following subsections walk through how each type enters the DPS and where it ends up in the metadata service.
...
- **Administrative metadata** is provided when you create a submission. A contract ID identifies which submission agreement governs the submission (see [role-based access control](/docs/dps/access-control/)), and an object ID uniquely identifies this object within your submission agreement. Both are stored in the preservation metadata database, enriching the Intellectual Entity document.
...
- **Extracted technical metadata** is produced for every file using MediaInfo and ExifTool. These extraction outputs are stored as files in the S3 object storage, referenced by the File documents. They capture technical properties of the files: resolution, bitrate, color space, and more. They are put in object storage for easier retrieval and possible use by analytical processing tools.
...
- **Agents**: the people, organisations, software, or hardware that performed each event.

For formal PREMIS definitions of these entity types, see [The role of DPS in the NLN](/docs/dps/context/).
...
- **Versioned** (planned): Dublin Core metadata will be updateable through the API, with each change recorded as an auditable version. This is not yet available.
- **Appendable**: PREMIS events can only be added, never modified. New events are appended to the existing record. A custodian can trigger re-extraction of technical metadata for individual files, producing a new extraction output while keeping the old.
- **Immutable**: SIP metadata files are frozen at submission and cannot be changed as of now.
